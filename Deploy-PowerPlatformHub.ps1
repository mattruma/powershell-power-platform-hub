...
try {
  ...
  try {
    ...
    if (!$site) {
      ...
      Connect-PnPOnline $newSiteURL -Interactive -ClientId $ClientId 
      ...
      ## Import Template
      Invoke-PnPSiteTemplate -Path template.pnp -Parameters @{'CompanyName' = $CompanyName; 'Year' = $year; 'Month' = $month }
